...
- shadcn/ui is configured by `app/components.json`, with source components checked into `app/src/mainview/components/ui/`. Added shadcn components should be treated as local source, reviewed after generation, and adapted to Level5 tokens instead of using `dark:` overrides or stock fixed surfaces. Level5 app-level wrappers live in `app/src/mainview/components/level5.tsx`; use those wrappers for repeated product controls such as icon buttons, sidebar buttons, surface cards, and status/attachment badges instead of restyling raw HTML at each call site.
