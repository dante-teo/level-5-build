...
`Level5Core` is the provider-neutral module where reusable runtime/domain code will grow. It currently owns recent-project persistence through GRDB, Git status/review services, and ACP protocol primitives. `Level5Design` owns native SwiftUI design primitives and reusable design resources. `Level5BuildApp` is the SwiftUI app target. The current UI is a native shell backed by an app-private agent session lifecycle model: a `NavigationSplitView` sidebar/detail layout, native window titlebar and command menus, startup session listing, selectable/deletable session rows, an unsent New Chat draft state, transcript replay, a native tokenized composer, per-session prompt queues, a new-session-only project picker, and an inspect-only Review column for selected/project-backed Git working trees.

`Level5BuildApp` also depends on SwiftUI Introspect for narrow AppKit interop. It is currently used by `TranscriptView` to inspect the backing `NSScrollView` so follow-tail behavior can use real scroll metrics instead of SwiftUI layout-preference guesses. It also depends on `MarkdownUI` (`swift-markdown-ui`) to render chat message bodies as real Markdown rather than raw text, themed via `L5MarkdownTheme` to match `Level5Design`'s typography and color tokens.
...
Once connected, the app initializes ACP and discovers mock model/slash-command metadata. There is no ACP `session/list` call anywhere in the app: the sidebar is sourced entirely from the durable local session cache (see Local persistence below), so a session this app never sent through — another client, another machine, or one lost to a crash before its first persisted write — is permanently invisible here. New Chat remains an unsent draft and creates no hidden ACP session until the first send. First send calls `session/new`, applies the selected New Chat model with `session/set_config_option` only when it differs from the reported model, inserts/selects the session row, then sends structured `session/prompt` content blocks; later sends reuse that `sessionId`. Selecting a row is pure retrieval: it paints that session's durably cached transcript and never talks to the agent runtime. Server-side context is fetched lazily instead, the moment the user actually sends into a session that this process hasn't already primed this run: that first send calls `session/load` to prime the backend's context, adopts its session model config, and then proceeds with the normal `session/prompt` — but any `session/update` replay the load triggers as a side effect is fully suppressed and never reaches the transcript or cache, since priming is a context-loading call, not a way to repaint history. A session created or primed earlier in the current process run is sent to directly, with no repeated `session/load`. Selection itself does not change sidebar recency; only sent or received live message activity updates app-observed recency. Delete is available from each sidebar row's context menu as `Delete Chat...`, requires native destructive confirmation, best-effort uses ACP `session/delete`, and returns to New Chat if the deleted session was active.
...
While a turn is running, the composer send button becomes an enabled Stop action. Stop immediately restores composer editing for the selected session, sends ACP `session/cancel`, answers any pending permission request with `{ "outcome": { "outcome": "cancelled" } }`, clears queued prompts for that same session, preserves already-streamed transcript content, records a compact cancelled status internally only (statuses are never rendered as transcript rows), and suppresses stale late output from the cancelled turn. If the user immediately sends another prompt in the same session, stale-output suppression stays active until the backend echoes the new prompt's user message; that echo is the boundary after which live output is accepted again. Manual Stop keeps a healthy ACP connection alive.
...
Transcripts are stored as app-private structured state, not flat local role/text rows. ACP updates normalize into deterministic transcript events for message chunks, plans, tool calls, usage, statuses, errors, stop reasons, and best-effort references. Messages merge by `messageId` when available and fall back to contiguous same-role merging without an ID; message bodies render as Markdown (bold/italic, inline code, links, lists, headings, blockquotes, code blocks) via `L5MarkdownTheme`, a `MarkdownUI` theme built from the `Level5Design` typography/color tokens, rather than as raw text. Unsupported non-text blocks are retained as compact unsupported-block counts. Plans and usage are session metadata rather than transcript rows: plans render as a centered composer-adjacent `Plan N/M` chip with a checklist popover, and usage renders only as the animated context ring beside the model selector. Tool calls and errors render as compact operational transcript rows; tools auto-expand while in progress, auto-collapse when completed unless manually expanded, and remain expanded on failure. Expanded tool rows show normalized status, kind, and readable detail text rather than raw ACP payloads. Statuses (runtime diagnostics, raw stderr, permission audit notes, notable stop reasons like `cancelled`/`refusal`/`max_tokens`) are recorded and persisted like any other transcript item but are never rendered as rows, regardless of source; raw stderr lines are additionally pre-filtered by `AcpRuntimeLogLine` so routine `TRACE`/`DEBUG`/`INFO` runtime tracing noise never even reaches persisted state (only `WARN`/`ERROR`/unstructured lines are recorded, though none of it is currently user-visible either way).
...
Project-backed sessions are intentionally narrower than "any backend cwd". A new session is project-backed only when first sent from an explicit selected recent project; the home-directory fallback is not project-backed. A persisted row rehydrated on a later launch is still recognized as project-backed when Devin's own project key (its normalized cwd) matches a persisted recent project; the mock backend's shared project key never does. Backend `cwd` values are never auto-added to recents.
...
Sessions and their transcripts also persist durably, through `Level5Core.SessionPersistenceStore` sharing the same SQLite database. `AgentSessionModel.hydrateAllPersistedSessions()` loads every known session, across every project, into the sidebar as a single global list on `start()` — the sidebar is never scoped to just the project currently selected for the next new chat, so a session from any project stays visible without reselecting that project. Selecting an existing row repaints its cached transcript instantly from disk without contacting the agent runtime at all. Deleting a session removes it locally and durably (`hidden_sessions`) so it cannot resurface even for backends, like real Devin, that don't support server-side deletion. See `docs/ARCHITECTURE.md` ("Durable session/transcript persistence") for the full schema and hydration/pruning details.
