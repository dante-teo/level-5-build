...
Stack: [Bun](https://bun.sh) (runtime + main process), [Electrobun](https://blackboard.sh/electrobun) (desktop app shell using the OS's native webview — WKWebView on macOS, WebView2 on Windows, webkit2gtk on Linux — not a bundled Chromium/CEF; `bundleCEF` is explicitly disabled for all platforms), React 18, Jotai for webview UI state, [`use-stick-to-bottom`](https://github.com/stackblitz-labs/use-stick-to-bottom) for the transcript's auto-scroll/stick-to-bottom behavior, Vite 6 (bundles the webview UI), Tailwind CSS v4, and New York-style shadcn/Radix primitives adapted to the Level5 design tokens. Package management and script execution both use Bun uniformly (no pnpm).
...
- **Pure renderer policy** (`app/src/mainview/lib/layout.ts`, `sessionGroups.ts`): deterministic inspector-fit/priority and cached-session grouping rules, kept outside bridge-importing React modules so they remain directly testable under Bun.
- **Adaptive drawer behavior** (`app/src/mainview/lib/useDrawerFocusTrap.ts`): shared Escape, focus wrapping, and prior-focus restoration for compact Dashboard and Review drawers. The boundary calculation is pure and regression-tested, including reverse tabbing from the drawer pane itself.
...
The source artwork lives at `app/assets/AppIconSource.png`; `app/assets/icon.png`, the macOS bundle icon at `app/assets/App.icns`, and the in-app PNG are generated derivatives. A post-build hook (`app/scripts/apply-macos-icon.ts`) copies the already-built `App.icns` into the app bundle before codesigning/notarization and updates the bundle plist icon keys. It does not generate icon artwork or `.icns` files. This avoids relying on Electrobun's `.iconset` conversion path, which depends on `iconutil` behavior on the build host.
...
- The liquid-glass app frame is tokenized in CSS: `.l5-liquid-pane`, `.l5-frame-top-gradient`, `.l5-sidebar-toggle-shell`, and `.l5-top-control` provide the floating sidebar pane, translucent top fade, animated sidebar affordance, and sparse circular Dashboard/Review controls. Keep glass selective; transcript and inspector content use quieter surfaces.
...
- `startAgentPrompt({ prompt, cwd, model, approvalMode, planMode, attachments })`: accepts a non-empty prompt and starts the ACP prompt flow if no turn is already running. `attachments` are sent as `resource_link` content blocks alongside the text block (URIs are percent-encoded via `pathToFileURL` in `app/src/bun/agent/promptContent.ts`). When `planMode` is true, the prompt text block is prepended with a planning-only system instruction (`PLAN_MODE_INSTRUCTION` in `promptContent.ts`) that tells the agent to research and propose a plan without making edits, then stop; on turn completion the webview extracts the agent's last message as the proposed plan and surfaces a `PlanEscalationPrompt` offering "Implement", "Clear context and implement", or "Reject with feedback" — the queue drains only on the failure branches (no plan returned, non-`end_turn` stop, error), not after a successful escalation, so the user's choice runs before any queued prompts. For the `devin` provider, `approvalMode` maps to process flags: `ask` and `auto` use `--permission-mode normal`; `full-access` uses `--permission-mode bypass`. The `omp` provider takes no per-launch permission-mode flag — its approval behavior comes from `omp`'s own config (`tools.approvalMode` / CLI overrides), not this app's approval-mode selector; ACP `session/request_permission` still drives the same auto-approve/surface-to-user logic either way.
...
The window is frameless: `titleBarStyle: "hiddenInset"` (native traffic lights, no visible title bar strip, `FullSizeContentView` so the webview covers the whole window). Native traffic-light placement and renderer top-control geometry share `app/src/shared/windowChrome.ts`; the native offset is passed as `trafficLightOffset` during `BrowserWindow` creation and re-applied with `setWindowButtonPosition`. This is native window state, not CSS: frontend HMR can refresh the webview without re-running the Bun-side offset call, so a development window may temporarily snap back to the platform default until the Electrobun app is restarted or the main process reapplies the position.
