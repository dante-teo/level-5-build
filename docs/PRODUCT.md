...
The sidebar shows an `All chats` section backed by app-side in-memory session summaries and updated as chats are created, loaded, or deleted. Startup session listing stays lazy so opening the app does not spawn or auth-probe Devin. Selecting a chat restores the cached transcript when available, or reloads persisted messages from ACP after relaunch; right-clicking a chat exposes delete with confirmation. This is not yet a durable production-grade session store.
