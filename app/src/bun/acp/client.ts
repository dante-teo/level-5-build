...
export const ACP_REQUEST_TIMEOUTS_MS = {
	setup: 15_000,
	// session/prompt doesn't resolve until the entire turn completes, which
	// routinely exceeds a short RPC default for any real tool-using turn.
	// This must stay a generous last-resort safety net against a leaked
	// continuation, not the mechanism that detects a stuck turn -- that's
	// AcpTurnIdleWatchdog's job (see watchdog.ts). Matches the native
	// AcpClient.swift's 6h override (see AGENTS.md).
	prompt: 6 * 60 * 60 * 1000,
	extension: 30_000,
} as const;
