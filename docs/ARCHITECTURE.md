...
Known Devin ACP gap: `session/delete` is not implemented by the real agent (`Method not found`), unlike the mock server. Deleting a Devin-backed session from the sidebar will surface an error until this is addressed upstream or worked around.
